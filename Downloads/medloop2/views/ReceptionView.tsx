...
const ReceptionView: React.FC = () => {
  ...
  useEffect(() => {
    if (!user) return;
    const unsubscribe = PatientService.subscribe(user, (data) => setPatients(data));
    return () => unsubscribe();
  }, [user]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        await PatientService.add(user, {
            name: formData.name, age: parseInt(formData.age), phone: formData.phone, gender: formData.gender,
            medicalProfile: {
                ...
                isPregnant: formData.gender === 'female' && formData.isPregnant
            },
            currentVisit: { visitId: '', clinicId: formData.clinicId, date: Date.now(), status: 'waiting', priority: formData.priority, source: formData.source, reasonForVisit: formData.reasonForVisit }
        });
        setFormData(prev => ({ name: '', age: '', phone: '', gender: 'male', allergiesExists: false, allergiesDetail: '', chronicExists: false, chronicDetail: '', medsExists: false, medsDetail: '', isPregnant: false, clinicId: prev.clinicId, priority: 'normal', source: 'walk-in', reasonForVisit: '' }));
        setIsFormOpen(false);
